...
int findClosest(vector<double>& arr, int n, int target)
{
	// Corner cases 
	if (target <= arr[0])
		return arr[0];
...
int main()
{
	...
	for (int i = 0; i != n; ++i)
	{
		...
		values.push_back(val); //populate a distribuiton slightly more interesting than uniform, with a lower density at higher values.
	}
	...
	{
		int temp = values[i];
		int closest = findClosest(resultingbreaksArray, k, temp);

		final = final + abs(temp - closest);
	}

	int finalVal = 25 * n - final;
	cout << "final is" << finalVal;
} // main 